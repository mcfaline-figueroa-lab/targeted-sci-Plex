...
cds.pre.path.1.dt = file.path(input_dir, "cds_precell_prehash_exp1_dt_700rpc.RDS")
cds.pre.path.1.targ = file.path(input_dir, "cds_precell_prehash_exp1_ff_700rpc.RDS")
cds.pre.path.2.dt = file.path(input_dir, "cds_precell_prehash_exp2_dt_700rpc.RDS")
cds.pre.path.2.targ = file.path(input_dir, "cds_precell_prehash_exp2_ff_700rpc.RDS")
...
# Save intermediate (Optional, commented out to save space/time unless needed)
# saveRDS(cds_ff, file.path(output_dir, "cds_pre_mouse_brain_ff_no_filt.RDS"))
# saveRDS(cds_dt, file.path(output_dir, "cds_pre_mouse_brain_dt_no_filt.RDS"))
...
analyze_cds_quality <- function(cds, plot_knee = TRUE, umi_cutoff = NULL, plot_title = NULL, output_path = NULL) {
  ...
  summary_df <- colData(cds) %>%
    as.data.frame() %>%
    group_by(method) %>%
    dplyr::summarize(
      med_umi = median(n.umi),
...
cds_dt <- analyze_cds_quality(cds_dt, umi_cutoff=40, plot_title = "dT randomN RT", output_path = file.path(output_dir, "plots/dt_knees_subsampled.png"))
cds_ff <- analyze_cds_quality(cds_ff, umi_cutoff=25, plot_title ="targeted RT", output_path = file.path(output_dir, "plots/ff_knees_subsampled.png"))
...
cds_ff_700rpc <- filter_and_preprocess_cds(cds_ff, umi_cutoff = 40, probe_genes = probe_genes, save_plot_path = file.path(output_dir, "QC/elbow_ff.png"))
cds_dt_700rpc <- filter_and_preprocess_cds(cds_dt, umi_cutoff = 25, probe_genes = probe_genes, save_plot_path = file.path(output_dir, "QC/elbow_dt.png"))

# --- Reductions & Alignment ---

# DT
cds_dt_700rpc <- detect_genes(cds_dt_700rpc)
expressed_genes_dt <- as.data.frame(rowData(cds_dt_700rpc)) %>% filter(num_cells_expressed > (0.05 * length(colData(cds_dt_700rpc)$cell))) %>% pull(id)
cds_dt_700rpc <- preprocess_cds(cds_dt_700rpc, num_dim = 9, use_genes = unique(c(expressed_genes_dt, probe_genes))) 
cds_dt_700rpc <- reduce_dimension(cds_dt_700rpc, preprocess_method = "PCA", umap.min_dist = 0.1, umap.n_neighbors = 25)
cds_dt_700rpc <- align_cds(cds_dt_700rpc, preprocess_method = "PCA", alignment_group ="experiment")
cds_dt_700rpc <- reduce_dimension(cds_dt_700rpc, preprocess_method = "Aligned", umap.min_dist = 0.1, umap.n_neighbors = 25)

p_dt <- plot_cells(cds_dt_700rpc, scale_to_range = FALSE, graph_label_size = 20, color_cells_by = "experiment") + facet_wrap(~experiment)  
ggsave(file.path(output_dir, "QC/aligned_UMAP_dt.png"), plot = p_dt, height = 5, width = 6, dpi=600)

# FF
expressed_genes_ff <- as.data.frame(rowData(cds_ff_700rpc)) %>% filter(num_cells_expressed > (0.05 * length(colData(cds_ff_700rpc)$cell))) %>% pull(id)
cds_ff_700rpc <- preprocess_cds(cds_ff_700rpc, num_dim = 9, use_genes = unique(c(probe_genes, expressed_genes_ff))) 
cds_ff_700rpc <- reduce_dimension(cds_ff_700rpc, preprocess_method = "PCA", umap.min_dist = 0.1, umap.n_neighbors = 25)
cds_ff_700rpc <- align_cds(cds_ff_700rpc, preprocess_method = "PCA", alignment_group ="experiment")
cds_ff_700rpc <- reduce_dimension(cds_ff_700rpc, preprocess_method = "Aligned", umap.min_dist = 0.1, umap.n_neighbors = 25)

p_ff <- plot_cells(cds_ff_700rpc, scale_to_range = FALSE, graph_label_size = 20, color_cells_by = "experiment") + facet_wrap(~experiment)
ggsave(file.path(output_dir, "QC/aligned_UMAP_targeted.png"), plot = p_ff, height = 5, width = 6, dpi=600)

save_monocle_objects(cds_dt_700rpc, file.path(output_dir, "cds/cds_dt_aligned_UMAP"))
save_monocle_objects(cds_ff_700rpc, file.path(output_dir, "cds/cds_ff_aligned_UMAP"))
